...
public class Mutate {
    ...
    public static void main(String[] args) {
        ...
        try {
			Files.copy((new File(local + "/scripts/comp_remove.sh")).toPath(), (new File(mutomvo + "/project_" + app + "/comp_remove.sh")).toPath(), StandardCopyOption.REPLACE_EXISTING);
		} catch (IOException e) {
...
//    			Process p = Runtime.getRuntime().exec("java -jar " + mutomvo + "/dist/mutomvo.jar"); // This for normal screens
		...
		try {
			...
			while (reader.readLine() != null) tests++;
			reader.close();
		} catch (IOException e) {
